...
local minPathSpacingSqDefault = 50 * 50
local spacingByCmd = {
	[CMD_MANUALFIRE] = 25*25,
	[CMD_AIR_MANUALFIRE] = 25*25,
}
...
local cancelNoPathable = { -- cancel totally when noPathable
	[CMD_JUMP] = true,
}
...
local positionCmds  =  {
	...
	[CMD.PATROL] = true,      [CMD.CAPTURE] = true,     [CMD.FIGHT] = true,      [CMD_MANUALFIRE] = true, [CMD_JUMP] = true,         [CMD.RESURRECT] = true,
	[CMD.UNLOAD_UNIT] = true, [CMD.UNLOAD_UNITS] = true,[CMD.LOAD_UNITS] = true, [CMD.GUARD] = true,      [CMD.AREA_ATTACK]  =  true,
	[CMD_AIR_MANUALFIRE] = true,

}
local acquiredTarget
local immobileDefID = {}
local hasImmobile
...
local function SetColor(cmdID, alpha)
	...
	elseif cmdID == CMD_MANUALFIRE             then glColor(1.0, 1.0, 1.0, alpha) -- White
	elseif cmdID == CMD_AIR_MANUALFIRE         then glColor(1.0, 1.0, 1.0, alpha) -- White
...
local function GetCmdOpts(alt, ctrl, meta, shift, right)
	...
	-- Echo('alt', alt, 'ctrl',ctrl, 'meta',meta, 'shift',shift,'right', right)
	opts.coded = coded
	return opts
end
...
local function TweakTarget(pos, mx, my, acquiredTarget, singleNode)
...
local function TweakCommand(usingCmd, targType, alt, ctrl, meta, shift, forceShift, forceAlt, usingRMB)
	local tweaked 
	-- if hasImmobile then
	-- 	return tweaked, usingCmd, alt, ctrl, meta, forceShift or shift
	-- end
	-- Echo("usingCmd,CMD_UNIT_SET_TARGET, CMD_UNIT_SET_TARGET_CIRCLE is ", usingCmd,CMD_UNIT_SET_TARGET, CMD_UNIT_SET_TARGET_CIRCLE)
	-- if (usingCmd == CMD_UNIT_SET_TARGET or usingCmd == CMD_UNIT_SET_TARGET_CIRCLE) 
	-- 	and targType=='ground' then
	-- 	Echo("shift,ctrl is ", shift,ctrl)
	-- end


	if (usingCmd == CMD_ATTACK) and not ctrl then
		if shift and not hasBomber and (not commandMap or commandMap['Set Target']) then
			usingCmd = CMD_UNIT_SET_TARGET
			ctrl = true
			tweaked = true
		elseif pathCandidate then
			ctrl = true -- make the alt+attack path use ctrl to trigger the 'shoot once' command along the line
		-- 	meta = true
		-- 	forceShift = true
			tweaked = true
		end
	elseif (usingCmd == CMD_UNIT_SET_TARGET or usingCmd == CMD_UNIT_SET_TARGET_CIRCLE) 
		and targType=='ground' and shift and not ctrl then

		ctrl = true -- make the path use ctrl to have a 'shoot toward' mod
		-- meta = true
		-- forceShift = true
		tweaked = true

	elseif (usingCmd == CMD_MANUALFIRE or usingCmd == CMD_AIR_MANUALFIRE) and pathCandidate then
		ctrl = false-- spam manual fire at different location following the mouse
		meta = false
		forceShift = false
		tweaked = true
	end
...
local function SetOrder(targType, forceShift, forceAlt, usingRMB, pos, tweakTarget, mx, my, acquiredTarget)
	local  alt, ctrl, meta, shift = GetModKeys()
	local tweaked
...
function widget:MousePress(mx, my, mButton, byEz)
	...
	-- 	Echo('namecom is Attack !',os.clock())
	-- end
	if activeCmdID then
		...
		local EzTarget = WG.EzTarget
		acquiredTarget = EzTarget and EzTarget.v.acquiredTarget
		...
		if not defaultCmdID then
			local targType, targID = CulledTraceScreenRay(mx, my, false, false)
			if targType then
		...
		if overrideCmdID then
			...
			elseif targType == 'feature' then
				...
				-- if nameDefCom == 'Attack' then
				-- 	Echo('CF2 let the Area Attack pass ! 2')
				-- end
	...
	-- 	-- pathCandidate = false
	-- 	-- -- if meta then
	-- 	-- -- 	-- test anti lag: as the pathing is not satifying with meta we don't use it
	-- 	-- -- 	-- local cmdOpts = GetCmdOpts(alt, ctrl, meta, shift, usingRMB) -- using alt uses springs box formation, so we set it off always
	...
	-- 	-- -- end
	-- 	if cancelNoPathable[usingCmd] then
	-- 		StopCommandAndRelinquishMouse()
	-- 		return false
	-- 	end
	-- end
...
function widget:MouseMove(mx, my, dx, dy, mButton)
	...
	-- Get clicked position
	local targType, pos = CulledTraceScreenRay(mx, my, true, inMinimap)
	...
	if fNodes[2] and not fNodes[3] then
		...
		if pathCandidate then
			...
			usingCmd, pos, cmdOpts = SetOrder(targType, forceShift, forceAlt, usingRMB, pos, tweakTarget, mx, my, acquiredTarget)
			GiveNotifyingOrder(usingCmd, pos, cmdOpts)
...
function widget:MouseRelease(mx, my, mButton)
	...
	if singlePoint then
		...
		-- If the line is a path, start the units moving to this node
			local forceShift, forceAlt = shift, false
			local tweakTarget = true
			usingCmd, pos, cmdOpts = SetOrder(targType, forceShift, forceAlt, usingContextCommand, pos, tweakTarget, mx, my, acquiredTarget)
			GiveNotifyingOrder(usingCmd, pos, cmdOpts)
		end
	end
	...
	elseif usingFormation then
		...
		-- Using formation? If so then it's time to calculate and issue orders.
			-- Echo('using formation', math.round(os.clock()))
		...
		local cmdOpts = GetCmdOpts(alt, ctrl, meta, shift, usingContextCommand)
		do
			local  alt, ctrl, meta, shift, usingContextCommand = alt, ctrl, meta, shift, usingContextCommand
			local forceShift, forceAlt = shift, alt
			local tweaked
			tweaked, usingCmd,  alt, ctrl, meta, shift, usingContextCommand = TweakCommand(usingCmd, targType, alt, ctrl, meta, shift, forceShift, forceAlt, usingContextCommand)
			if tweaked then
				cmdOpts = GetCmdOpts(alt, ctrl, meta,shift, usingContextCommand)
			end
		end
		-- Single click ? (no line drawn)
		--if (#fNodes == 1) then
		local mult = os.clock() - clickTime < 0.1 and 3 or 1 -- give more leeway if the click time was very short

		if fDists[#fNodes] < minFormationLength * mult then
			...
			local params = TweakTarget(fNodes[1], mx, my, acquiredTarget, true)
...
function widget:KeyRelease(key)
	if (key == keyShift) and endShift or key == keyAlt and endAlt then
...
function widget:Initialize()
	...
	InitFilledCircle(CMD_MANUALFIRE)
	InitFilledCircle(CMD_AIR_MANUALFIRE)
