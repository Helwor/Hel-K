...
local function GetSingleUnitInfoPanel(parentControl, isTooltipVersion)
	...
	local function UpdateDynamicUnitAttributes(unitID, unitDefID, ud)
		...
		if shieldBarUpdate then
			if ud and ud.shieldPower and (ud.shieldPower > 0 or ud.level) then
...
local function GetTooltipWindow()
	...
	local externalFunctions = {visible = false}
	...
	function externalFunctions.SetVisible(newVisible)
		...
		externalFunctions.visible = newVisible
	end
...
local function UpdateSelection(newSelection)
	...
	if not (newSelection and newSelection[1]) then
		selectionWindow.SetVisible(false)
		return
	end
	
	selectionWindow.SetVisible(true)
	if not newSelection[2] then
...
function widget:Update(dt)
	...
	-- Echo("WG.panning is ", WG.panning)
	if not WG.panning then
		UpdateTooltip(dt, not slowUpdate)
	elseif tooltipWindow.visible and screen0.currentTooltip == NO_TOOLTIP then
		tooltipWindow.SetVisible(false)
	else
		-- UpdateTooltip(dt, not slowUpdate)
	-- elseif not hidden then --if screen0.currentTooltip ~= NO_TOOLTIP then
	-- 	Echo("screen0.currentTooltip is ", screen0.currentTooltip)
	-- 	screen0.currentTooltip = 'test'
	-- 	UpdateTooltip(dt, not slowUpdate)
	-- 	-- screen0.currentTooltip = NO_TOOLTIP
	-- 	hidden = true
