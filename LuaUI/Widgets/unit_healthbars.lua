...
local paraUnits   = {}
WG.disarmUnits = WG.disarmUnits or {}
local disarmUnits = WG.disarmUnits
for id in pairs(disarmUnits) do
	disarmUnits[id] = nil
end
local onFireUnits = {}
local UnitMorphs  = {}
...
local function showhealthbars(cmd, line, words)
	if ((words[1])and(words[1] ~= "0"))or(deactivated) then
		...
		widgetHandler:RemoveWidgetCallIn('DrawWorld',widget)
		deactivated = true
	end
end
...
do
	...
	function JustGetOverlayInfos(unitID, unitDefID)
		...
		-- 		disarmUnits[unitID] = not stunned
		-- 	end
		-- end
		if not stunned and disarmUnits[unitID]==false then
			disarmUnits[unitID] = true
		end
	end

	function DrawUnitInfos(unitID, unitDefID, unit)
		...
		local healthTable = unit.health
		if not (healthTable and healthTable[1]) then
			local defID = spGetUnitDefID(unitID)
			local name = defID and UnitDefs[defID].name
			Echo('Problem at HealthBars, no health table for unit',unitID,'unit?',unit,'healthTable?',healthTable,'name?',name,'dead?',spGetUnitIsDead(unitID))
			health, maxHealth, paralyzeDamage, capture, build = GetUnitHealth(unitID)
		else
			health, maxHealth, paralyzeDamage, capture, build = unpack(healthTable)
		end
		paralyzeDamage = GetUnitRulesParam(unitID, "real_para") or paralyzeDamage
...
do
	...
	function DrawFeatureInfos(featureID, featureDefID, addPercent, addTitle, fx, fy, fz)
		...
		local _, health, maxHealth, resurrect, reclaimLeft
		if not feature.health or updateRes then
			...
			feature.health = {health, maxHealth, resurrect, reclaimLeft}
		else
			health, maxHealth, resurrect, reclaimLeft = unpack(feature.health)
		end
...
do
	...
	function DrawOverlays()
		...
		if (drawStunnedOverlay) and ((paraUnits[1]) or next(disarmUnits)) then
			glDepthTest(true)
			glPolygonOffset(-2, -2)
			glBlending(GL_SRC_ALPHA, GL_ONE)
			local alpha = ((5.5 * time) % 2) - 0.7
			if (paraUnits[1]) then
			...
			if next(disarmUnits) then
				glColor(0.8, 0.8, 0.5, alpha/6)
				for id, bool in pairs(disarmUnits) do
					if bool then
						glUnit(id, true)
					end
				end
		...
		if (drawUnitsOnFire)and(onFireUnits[1]) then
			glDepthTest(true)
			glPolygonOffset(-2, -2)
			glBlending(GL_SRC_ALPHA, GL_ONE)

			local alpha = abs((time % 2)-1)
...
function widget:Initialize()
	...
	widgetHandler.actionHandler:AddAction(widget, "showhealthbars", showhealthbars)
	...
	widgetHandler:RegisterGlobal(widget, 'MorphDrawProgress', function() return true end)
...
function widget:Shutdown()
	...
	--// catch f9
	widgetHandler.actionHandler:RemoveAction(widget, "showhealthbars", showhealthbars)
	...
	widgetHandler:DeregisterGlobal(widget, 'MorphDrawProgress')
end
...
local visibleUnits = {}
local updateFeatures = true
local GetUnitRulesParam = Spring.GetUnitRulesParam
local UpdateDisarmUnits = function()
	for id in pairs(disarmUnits) do
		disarmUnits[id] = nil
	end
...
do
	...
	function widget:DrawWorld()
		...
		if not Spring.IsGUIHidden() then
			...
			-- 	disarmUnits[id] = nil
			-- end
			if (not visibleUnits[1] and not  visibleFeatures[1]) then
				return
			end
			...
			for i = 1, #visibleUnits do
				...
				if unit then
					...
					if DrawUnitInfos(unitID, unitDefID, unit) then
						-- local x, y, z = Spring.GetUnitPosition(unitID)
						local x, y, z = unit:GetPos(3)
			...
			local wx, wy, wz, dx, dy, dz, dist,  valid
			local featureInfo
			for i = 1, #visibleFeatures do
				...
				if (valid) then
					...
					if (dist < featureDistSq) then
						DrawFeatureInfos(featureInfo.id, featureInfo.defID, (dist < featurePercentSq), (dist < featureTitleSq), wx, wy, wz)
					end
				end
			end
...
do
	...
	function widget:Update(dt)
		...
		visibleUnits = Visibles.not_icons --this don't need any delayed update or caching or optimization since its already done in "LUAUI/cache.lua"

		sec2 = sec2+dt
		sec3 = sec3 + dt
		sec4 = sec4 + dt
		if sec4>UPDATE_RES_TIME then
			updateRes = true
			sec4 = 0
		end
		if sec3>0.15 then
		...
		if (sec2 > 1/3) then
			...
			for i = cnt, 1, -1 do
				...
				local feature = Features[featureID]
				if not feature or updateFeatures then
					feature = {
						id = featureID,
						defID = GetFeatureDefID(featureID) or -1,
					}
					Features[featureID] = feature
				end

				--// filter trees and non destructable features
				featureDefID = feature.defID
				if destructableFeature[featureDefID] and (drawnFeature[featureDefID] or (select(5, GetFeatureResources(featureID)) < 1)) then
					if not feature[1] or updateFeatures then
						feature[1], feature[2], feature[3] = GetFeaturePosition(featureID)
					end 
					visibleFeatures[i] = feature
				else
...
function widget:FeatureDestroyed(id)
	Features[id] = nil
end
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
