...
local function GetHoveredUnit(dt) --Mostly a convenience function for the animation system
	...
	if unitID and not selMap[unitID] and Visibles.not_iconsMap[unitID] then
		local unit = Units[unitID]
...
local function CreateDisplayLists(callbackShape)
	...
	displayLists.select = callbackShape.hollow(selColor, outersize, selectinner) 
	displayLists.hover = callbackShape.hollow(teal, outersize, selectinner)

	-- displayLists.invertedSelect = callbackShape.hollow(colorout, selColor, outersize, selectinner)
	-- displayLists.inner = callbackShape.plain(zeroColor, innersize)
	displayLists.innermask = callbackShape.plain(nil, selectinner)
	displayLists.outermask = callbackShape.hollow(nil,outersize, selectinner)
	-- displayLists.kill = callbackShape.plain(nil, outersize)
	-- displayLists.shape = callbackShape.hollow(zeroColor, CAlphaColor, innersize, selectinner)
...
function widget:Update(dt)
	...
	if timePassed < UPDATE_RATE and not HasToDraw() then
		return
	end
	timePassed = 0
...
local function DrawUnitShapes(unitList, color, stencil)
	...
	if stencil then
		...
		for i = 1, #unitList do
			...
			if unit then
				...
				-- glCallList(unit.shape.innermask)
				-- glPopMatrix()
				glDrawListAtUnit(unitID, unit.shape.innermask, false, unit.xscale * scale, 1.0, unit.zscale * scale, degrot[unitID], 0, degrot[unitID], 0)
			end
		end
		-- glDepthMask(false)
	...
	for i = 1, #unitList do
		...
		if unit then
			glDrawListAtUnit(unitID, unit.shape.select, false, unit.xscale * scale, 1.0, unit.zscale * scale, degrot[unitID], 0, degrot[unitID], 0)
		end
	end
	glColor(color)
	glColorMask(true,true,true,true)
...
local function DrawShapes()
	...
	DrawUnitShapes(visibleSelected, rgba, 'select', true)
	...
	DrawUnitShapes(hoveredUnit, hoverColor, false)
