...
function widget:CommandNotify(cmdID, cmdParams, cmdOptions)
	...
	elseif TargetCancelingCommand[cmdID] and options.removeTarget.value and not (cmdOptions and cmdOptions.shift) then
		local units = Spring.GetSelectedUnits()
		if cmdID == CMD_STOP then -- only remove set target instead of a full stop if any unit got a target set without order attack on it
			for i = 1, #units do
				local unitID = units[i]
				if isValidUnit(unitID) then
					local target_type = (spGetUnitRulesParam(unitID,"target_type") or 0)
					if target_type  > 0  then
						local untargetting = true
						if target_type == UNIT_TARGET then
							-- verify the target is not also directly attacked by our subject
							local currCmdID, _, _, cmdParam1, cmdParam2 = Spring.GetUnitCurrentCommand(unitID)
							if currCmdID == CMD_ATTACK and cmdParam1 == spGetUnitRulesParam(unitID, "target_id") then
								untargetting = false
							end
						end
						if untargetting then
...
function widget:UnitCommandNotify(unitID, cmdID, cmdParams, cmdOpts)
	...
	elseif TargetCancelingCommand[cmdID] and options.removeTarget.value then
		if cmdID == CMD_STOP then
			if (spGetUnitRulesParam(unitID,"target_type") or 0) > 0  then
