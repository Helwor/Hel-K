...
function widget:CommandNotify(id, params, options)
	-- if id == CMD.GUARD and params[1] and not params[2] then
	...
	-- 			SetAutoCallTransportState(params[1], Spring.GetUnitDefID(params[1]), 1)
	-- 		end
	-- 	end
	-- 	return
	-- end
...
function widget:CommandsChanged()
	...
	for unitDefID, units in pairs(selectedSorted) do
		...
		if searchTransport and transportDef[unitDefID] then
			...
			-- Echo(os.clock(),units[1],'insert and set order unit AI',order)
...
function widget:Initialize()
	...
	for i = 1, #units do	-- init existing transports
		...
		AddTransport(unitID, spGetUnitDefID(unitID))
