...
function widget:GetInfo()
	return {
		--Helwor fixed income as digit not applying
...
-- default options for right click on mex => set active command mex
local autoMex = false
...
local glDrawCircle = gl.DrawCircle
...
local metalmult = tonumber(spGetModOptions().metalmult) or 1
...
local function PlaceSingleMex(bx, bz, facing, options)
	facing = facing or spGetBuildFacing() or 0
	...
	if closestSpot then
		...
		elseif foundUnit then
			...
			local commandHeight = math.max(0, spGetGroundHeight(closestSpot.x, closestSpot.z))
			if spTestBuildOrder(mexDefID, closestSpot.x, commandHeight, closestSpot.z, facing) == 0 then
				return true, true
			end
...
local function MakeMexTerraform(units, pointX, pointZ, height, holeMode)
	...
	local pointY = spGetGroundHeight(pointX, pointZ)
	...
	local team = spGetUnitTeam(units[1]) or spGetUnitTeam()
...
local function HandleAreaMex(cmdID, cx, cy, cz, cr, cmdOpts)
	...
	do --issue ordered order to unit(s)
		...
		for i, command in ipairs(orderedCommands) do
			local x = command.x
			local z = command.z
			local y = math.max(0, spGetGroundHeight(x, z))
			...
			if makeMexEnergy then
				for i = 1, energyToMake do
					...
					local buildDefID = (spTestBuildOrder(solarDefID, xx, yy, zz, 0) == 0 and windDefID) or solarDefID
...
local function CheckEnemyMexes(spotID)
	...
	local x = spotM.x
	local z = spotM.z
	local los = spGetPositionLosState(x, 0, z)
	if not los then
		return
	end
...
local function Initialize()
	if WG.metalSpots then
		spEcho("Mex Placement Initialised with " .. #WG.metalSpots .. " spots.")
		for i = 1, #WG.metalSpots do
			spotHeights[i] = WG.metalSpots[i].y
		end
	else
		spEcho("Mex Placement Initialised with metal map mode.")
	end
...
local function UpdateOctant()
	myOctant = GetAllyTeamOctant(spGetMyAllyTeamID()) or myOctant
	--spEcho("myOctant", myOctant, GetAllyTeamOctant(spEcho()))
end
...
function widget:Update(dt)
	...
	if firstUpdate then
		if spGetGameRulesParam("waterLevelModifier") or spGetGameRulesParam("mapgen_enabled") then
			Initialize()
			CheckAllTerrainChanges()
		end
...
local function getSpotColor(id)
	local teamID = spotData[id] and spotData[id].team or spGetGaiaTeamID()
...
local function calcMainMexDrawList()
	...
	for i = 1, #WG.metalSpots do
		...
		glDrawGroundCircle(x, --[[(preGame and spGetGroundHeight(x,z) or 0) +--]] 1, z, 40, 21)
		glColor(r,g,b,0.7)
...
function updateMexDrawList()
	...
	if circleOnlyMexDrawList then
		glDeleteList(circleOnlyMexDrawList)
	end
	if minimapDrawList then
		glDeleteList(minimapDrawList)
	end
	if incomeLabelList then
		-- trigger a refresh of incomeLabelList in Update as a camera rotation would do
		debounceCamUpdate = 0
		camDir = false
	end
	...
	if not circleOnlyMexDrawList then
		spEcho("Warning: Failed to update mex draw list.")
	end
end
...
function widget:DrawWorld()
	...
	if WG.metalSpots and (isMexCmd or pregame or ((WG.showeco or WG.showeco_always_mexes) and WG.selectionEntirelyCons) or CMD_AREA_MEX == cmdID or CMD_AREA_TERRA_MEX == cmdID) then
		...
		if closestSpot and wantShow and IsSpotBuildable(index) then
			...
			glLineWidth(1.49)
			glColor(1, 1, 0, 0.5)
			glBeginEnd(GL_LINE_STRIP, DoLine, bx, by, bz, closestSpot.x, height, closestSpot.z)
			glLineWidth(1.0)
			...
			glUnitShape(mexDefID, spGetMyTeamID(), false, true, false)
			glPopMatrix()
...
function widget:DefaultCommand(cmdType, id)
	if autoMex and mexSpotToDraw and not cmdType and (spTestBuildOrder(mexDefID, mexSpotToDraw.x, 0, mexSpotToDraw.z, 0) > 0) then
		return -mexDefID
	end
end
