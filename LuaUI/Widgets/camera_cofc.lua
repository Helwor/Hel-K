...
local scrollSmoothMeshAllowed = Game.mapName~='RandomMapGen12x12 1.6.12 ZK'
...
local cnt, time, maxcnt = 0, 0, 10
local lagging = false
local dtcounts = {total = 0, avg = 0}
for i=1,maxcnt do dtcounts[i] = 0 end
local time = 0
...
local function HotkeyChangeNotification()
	...
	key = WG.crude.GetHotkeyRaw("viewtaflip")
	key = key and key[1]
	local mods ={}
	if type(key) == 'string' then
		local keys = explode('+', key)
		keycode = ToKeysyms(table.remove(keys,#keys))
		for _,char in ipairs(keys) do
			mods[MODS[char]] = true
		end
		for _,mod in pairs(MODS) do
			mods[mod] = not not mods[mod]
		end
	else	
		keycode = ToKeysyms(key)
	end
	if keycode then
		key_code.viewtaflip = keycode
		key_code.viewtaflip_mods = mods
	end
end
...
GetDistForBounds = function(width, height, maxGroundHeight, mult, fov)
	local fittingDistance = height  / 2
...
local function GetSmoothOrGroundHeight(x,z,checkFreeMode) --only ScrollCam seems to want to ignore this when FreeMode is on
	if options.smoothmeshscroll.value and scrollSmoothMeshAllowed then
		...
		-- Echo(ret)
		return ret -- FIX: this is very wonky on RandomMapGen 12x12
	else
...
local function CorrectTraceTargetToSmoothMesh(cs,x,y,z)
	...
	local y_smooth = spGetSmoothMeshHeight(x, z) or spGetGroundHeight(x, z) or 0
	if y_smooth == 0 then -- ~= Helwor fix
		return x,y,z
	end
...
function SetLockSpot2(cs, x, y, useSmoothMeshSetting) --set an anchor on the ground for camera rotation
	...
	if useSmoothMeshSetting then
		-- ~= Helwor -- This report wrong height bc of smoothmesh gheight
...
function widget:Update(dt)
	...
	dtcounts.total = dtcounts.total - dtcounts[cnt] + dt
	dtcounts[cnt] = dt
	dtcounts.avg = dtcounts.total / maxcnt
	if time>=4 then
		-- Echo('refresh/sec:',('%.0f'):format(1/dtcounts.avg))
		time = 0
	end
...
local function FlipCamera()
	ls_have = false
	SetLockSpot2(GetTargetCameraState())
	RotateCamera(mcx, mcy, PI*2*100, 0, false, ls_have, false)
end
function widget:KeyPress(key, modifier, isRepeat)
	...
	-- 	Echo('not isRepeat',os.clock())
	-- end

	tilting = tilting and isRepeat --false -- NOTE: this to prevent the cam getting wonky -- tilting false will induce a new lockspot at ground, which might be off because the tilting was not finished (for lag or any reason)
	-- tilting = false
	...
	elseif key == key_code.viewtaflip then
		local passed = true
		for mod,bool in pairs(key_code.viewtaflip_mods) do
			passed = modifier[mod] == bool
			if not passed then
				break
			end
		end
...
function widget:Shutdown()
	...
	if ls_x then
		local cs = Spring.GetCameraState()
		cs.px,cs.py,cs.pz = ls_x,ls_y,ls_z
		cs.height = ls_dist
